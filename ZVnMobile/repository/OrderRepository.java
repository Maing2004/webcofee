...
public interface OrderRepository extends JpaRepository<OrderEntity, Long> {
	...
	@Query(value = "SELECT "
			+ "	   SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) THEN o.total_price ELSE 0 END) AS doanhthu,"
			+ "    SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status = 'Chờ xác nhận' THEN 1 ELSE 0 END) AS choxacnhan,"
			+ "    SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status = 'Đang lấy hàng' THEN 1 ELSE 0 END) AS danglayhang,"
			+ "    SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status = 'Đang giao hàng' THEN 1 ELSE 0 END) AS danggiaohang,"
			+ "    SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status = 'GH thành công' THEN 1 ELSE 0 END) AS ghthanhcong,"
			+ "    SUM(CASE WHEN EXTRACT(MONTH FROM o.created_at) = EXTRACT(MONTH FROM CURRENT_DATE) AND o.status = 'Đã hủy' THEN 1 ELSE 0 END) AS dahuy "
			+ "FROM vnmobilestore.orders o ", 
    nativeQuery = true)
	List<Object[]> getOrderStatisticsForCurrentMonth();
