...
public class OrderService implements IOrderService {
	...
	public DataResponse getOrderAdminSearchOrder(int status, int gateway, String keyword, int pageNumber) {
		...
		try {
			Pageable pageable = PageRequest.of(pageNumber - 1, 10);
			Page<OrderEntity> orderPage = orderRepository.findOrdersByStatusAndGatewayAndKeyword(statusStr, gatewayStr,
					keyword, pageable);
			List<OrderDto> listOrderDtos = new ArrayList<>();
			for (OrderEntity orderEntity : orderPage) {
				OrderDto orderDto = orderConverter.OrderEntityToOrderDto(orderEntity);
	...
	public DataResponse getOrderAdminSearchOrder2(int status, int gateway, String keyword, String startDate,
		...
		try {
			Pageable pageable = PageRequest.of(pageNumber - 1, 10);
			Page<OrderEntity> orderPage = orderRepository.adminFind2OrderSort(statusStr, gatewayStr, keyword,
					gatewayStr, startDateFinal, endDateFinal, pageable);
			List<OrderDto> listOrderDtos = new ArrayList<>();
			for (OrderEntity orderEntity : orderPage) {
				OrderDto orderDto = orderConverter.OrderEntityToOrderDto(orderEntity);
				listOrderDtos.add(orderDto);
	...
	@Override
	public DataResponse adminGetSaleOnCurrentMonth() {
		DataResponse dataResponse = new DataResponse();
		try {
			List<Object[]> results = orderRepository.getOrderStatisticsForCurrentMonth();
			if (results != null && !results.isEmpty()) {
//				Object[] result = results.get(0);
//				Double totalRevenue = (Double) result[0];
//				Long pendingCount = ((Number) result[1]).longValue();
//				Long successCount = ((Number) result[2]).longValue();
